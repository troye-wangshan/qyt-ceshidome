...
public class ServiceActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//System.out.print("第一处：setContentView");
		//Log.i(ServiceActivity.ACTIVITY_TAG, "第一处setContentView");
		setBroadcastReceiver();
		//System.out.print("第二处：setBroadcastReceiver");
		//Log.i(ServiceActivity.ACTIVITY_TAG, "第二处setBroadcastReceiver");
		initView();
		//System.out.print("第三处：initView");
		//Log.i(ServiceActivity.ACTIVITY_TAG, "第三处initView");
		getDefaultName();
		//System.out.print("第四处：getDefaultName");
		//Log.i(ServiceActivity.ACTIVITY_TAG, "第四处getDefaultName");
	}
	...
	private class BluetoothReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BLEService.ACTION_READ_Descriptor_OVER.equals(action)) {
				Log.i("BluetoothReceiver","intent.getIntExtra(\"value\", -1)="+intent.getIntExtra("value", -1));

				if (BluetoothGatt.GATT_SUCCESS == intent.getIntExtra("value",
						-1)) {
					Log.i(" BluetoothReceiver","read_name_flag = true;");
...
//				BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
	...
	// 获取名字
	private static boolean read_name_flag = false;//改动的地方，加了变成静态
//	private boolean servicesdiscovered_flag = false;
	...
	private class readNameThread extends Thread{
		...
		public void run() {
			...
			//Log.i(ServiceActivity.ACTIVITY_TAG, "读取设备名前");
			...
			//Log.i(ServiceActivity.ACTIVITY_TAG, "读取通道信息前");
			String uuid;
			...
			for (BluetoothGattService service : services) {
				...
				if(gattCharacteristics.size() == 0){
					//Log.i(ServiceActivity.ACTIVITY_TAG, "gattCharacteristics.size() == 0");
					continue;
				}
				// 添加一个一级目录
				...
				for (BluetoothGattCharacteristic gattCharacteristic : gattCharacteristics) {
					...
					//Log.i(ServiceActivity.ACTIVITY_TAG, "if (null != descriptor)之前");
					if (null != descriptor) {
						read_name_flag = false;

						//Log.i(ServiceActivity.ACTIVITY_TAG, "read_name_flag = false");
						Tools.mBLEService.mBluetoothGatt
								.readDescriptor(descriptor);
						Log.i(ServiceActivity.ACTIVITY_TAG, "等待读取完成之前");
						while (!read_name_flag) {// 等待读取完成
							//Log.i("read_name_flag", read_name_flag+"");
							try {
								sleep(50);
							} catch (InterruptedException e) {
								e.printStackTrace();
							...
							if (exit_activity || bind_flag){
								//Log.i(ServiceActivity.ACTIVITY_TAG, "等待读取完成");
								bind_flag = false;
								System.out.println("read fail");
								return; // 读取超时，结束线程
							}
						}
						//Log.i(ServiceActivity.ACTIVITY_TAG, "读取超时之后");
						try {
							child_data.put("name",
						...
						//Log.i(ServiceActivity.ACTIVITY_TAG, "if (null != descriptor)else");
						child_data
								.put("name", SampleGattAttributes.lookup(
										uuid, "unknow"));
					}
					//Log.i(ServiceActivity.ACTIVITY_TAG, "if (null != descriptor)之后");
					String pro = "";
				...
				//Log.i(ServiceActivity.ACTIVITY_TAG, "一级条目添加完成前");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(!Tools.mBLEService.isConnected()){
			//Log.i(ServiceActivity.ACTIVITY_TAG, "!Tools.mBLEService.isConnected()");
			finish();
		}
	}
