...
public class TalkActivity extends Activity implements OnClickListener {
	...
	/**
	 * byte[]转int(my_addfangfa)
	 *
	 * @return
	 */
	////////////////////////////////////////////////////////////////////////////////
	public static void  byteArrayToInt(byte[] bytes,short[] temp) {

		temp[0]= (short) (bytes[0]<<8|bytes[1]);
		temp[1]= (short) (bytes[2]<<8|bytes[3]);
	}
	...
	private void dis_recive_msg(byte[] tmp_byte) {
		...
		case 2: // 10进制显示
			...
			int count2 = 0;
			int count3 = 0;
			for (int i = 0; i < tmp_byte.length-4; i++) {
				count *= 256;//相当于左移8位
				count += (tmp_byte[i] & 0xFF);
			}
			for (int i = 2; i < tmp_byte.length-2; i++) {
				count2 *= 256;//相当于左移8位
				count2 += (tmp_byte[i] & 0xFF);
			}
			for (int i = 4; i < tmp_byte.length; i++) {
				count3 *= 256;//相当于左移8位
				count3 += (tmp_byte[i] & 0xFF);
			}
			float c= (float) (count*0.0625);
			float c2= (float) (count2*0.0625);
			float c3= (float) (count3*0.0625);

			String tmp1="";
			tmp1=String.format("%.2f",c);
			tmp2=String.format("%.2f",c2);
			tmp3=String.format("%.2f",c3);

//			String tmp1,tmp2="";
//			tmp1=Float.toString(c);
			//tmp2=Float.toString((float)(local_temp[1]*0.0625));
		...
		ChatMsgFmt entity2 = new ChatMsgFmt("设备", tmp, MESSAGE_FROM.OTHERS);
